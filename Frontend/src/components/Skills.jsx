      ...
      <div className="relative z-10 text-center max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-white/40 mb-16 max-w-xl mx-auto">
            Expertise in building scalable full stack web applications with modern
        ...
        >
          {skills.map((item, index) => (
            <motion.div
              key={index}
              className="group bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-6 hover:border-white/30 transition-all duration-300 flex flex-col items-center justify-center shadow-xl"
              whileHover={{ y: -5 }}
            >
              <div
                className="text-4xl mb-4 transition-all duration-300 group-hover:scale-110 group-hover:drop-shadow-[0_0_15px_rgba(0,212,255,0.8)]"
                style={{ color: item.color }}
              >
                {item.icon}
              </div>
              <p className="text-sm font-bold tracking-tight text-gray-200 group-hover:text-white transition-colors text-center uppercase">
                {item.name}
              </p>
            </motion.div>
          ))}
